(function () {
  ...
  function renderSummaryCard(card) { const auditClass = card.title === "Audit Quality" ? " audit-summary-card" : ""; return `<article class="summary-card${auditClass}"><span>${card.title}</span><strong>${card.value || "—"}</strong><small>${card.note || ""}</small></article>`; }
