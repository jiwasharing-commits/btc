(function () {
  ...
  function buildPanelData(options = {}) {
    const scenario = window.BtcDash.state?.scenarioContext?.primaryScenario; return { title: "Scenario Plan", rows: [{ label: "Primary", value: scenario?.label || "No scenario" }, { label: "Reference", value: scenario?.riskPlan?.quality || "Planning context only" }] };
  }
  ...
  window.BtcDash.ui.panels.scenario = { renderPanel, buildPanelData, renderEmptyState };
})();
